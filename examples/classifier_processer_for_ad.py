...
class MrpcProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, segment='train', combine=True):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train.tsv")), "train", combine=combine)

    def get_dev_examples(self, data_dir, segment='dev', combine=False):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev.tsv")), "dev", combine=combine)
    ...
    def _create_examples(self, lines, set_type, is_test=False, combine=True):
        ...
        for (i, line) in enumerate(lines):
            ...
            if is_test:
                ...
                label = line[0]
            examples[self.get_labels().index(label)].append(
...
class MnliProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir, segment='dev_matched', combine=False):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, segment+".tsv")), segment, combine=combine)
...
class SstProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, is_test=False, combine=True):
        """Creates examples for the training and dev sets."""
        examples = [[] for i in range(len(self.get_labels()))]  # examples with different golden label
        all_examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
